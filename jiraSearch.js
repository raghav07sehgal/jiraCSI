...
search(
  {
    ...
    jql: 'project = "CSI" AND status was "Inputs Required" AND assignee was in (aswalr) and createdDate >= "2018-3-27" AND createdDate< "2018-3-31"', // the JQL
            ...
            if (nextAsDate.length > 1) {
              ...
              for (let i = 0; i < fromDate.length; i++) {
                if (fromDate[i] == nextAsDate[i]) {
                  ...
                  var value_start = nextAsTime.split(':');
